...
try:
    from psana import (
        ...
        MpiDataSource,  # pyright: ignore[reportUnusedImport, reportAttributeAccessIssue, reportUnknownVariableType]  # noqa: F401
    )

    using_psana2: bool = True
...
def test_detector_interface() -> None:
    ...
    event_source: Generator[Any, None, None] = cast(
        Generator[Any, None, None], psana_run.events()
    )
    ...
    Psana2AreaDetector_raw: NDArray[numpy.signedinteger[Any]] = cast(
        NDArray[numpy.signedinteger[Any]], psana_run.Detector("jungfrau").raw.raw(event)
    )
    ...
    Psana2AssembledAreaDetector_raw: NDArray[numpy.signedinteger[Any]] = (
        psana_run.Detector("jungfrau").raw.image(event)
    )
    assert numpy.array_equal(
        Psana2AssembledAreaDetector_func, Psana2AssembledAreaDetector_raw
    )
    ...
    assert numpy.array_equal(Psana2PV_func, Psana2PV_raw)
    ...
    assert numpy.array_equal(Psana2EBeam_func, Psana2EBeam_raw)
    ...
    psana_run = next(ds.runs())

    event_source = cast(Generator[Any, None, None], psana_run.events())
    event = next(event_source)
    name = "Psana2Gmd"
    parameters.__pydantic_extra__ = {
        "type": "Psana2DetectorInterface",
    ...
    assert numpy.array_equal(Psana2Gmd_func, Psana2Gmd_raw)
    ...
    assert numpy.array_equal(Psana2Camera_func, Psana2Camera_raw)
